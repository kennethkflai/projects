...
if __name__ == "__main__":
    ...
    _argparser.add_argument(
        '--timestep', type=int, default=6, metavar='INTEGER',
        help='Time step in network')
    _argparser.add_argument(
        '--cycle', type=int, default=1, metavar='INTEGER',
        help='Number of cycles')
    _argparser.add_argument(
        '--base', type=int, default=1000, metavar='INTEGER',
    ...
    _argparser.add_argument(
        ...
        help='Learning rate')
    _argparser.add_argument(
        '--wd', type=float, default=0.001, metavar='FLOAT',
        help='Weight decay')
    _argparser.add_argument(
        '--batch_size', type=int, default=4, metavar='INTEGER',
        help='Batch Size')
    _argparser.add_argument(
        '--normalize_point', type=int, default=6, metavar='INTEGER',
        help='Normalize Skeleton Point')
    _argparser.add_argument(
        '--name', type=int, default=3, metavar='INTEGER',
    ...
    save_file = t_model.train(train_data, train_label, cv,
    ...
    for i in range(0, len(pred_label)):
        ...
        if model_type==1:
            ...
            f.write("%d, TS: %3.0f, model: %2d, BatchSize: %3d, Accuracy: %.2f%%\n"
                % (cv, num_frame, i, batch_size, 100*np.sum(temp)/len(temp)))
        f.close()
